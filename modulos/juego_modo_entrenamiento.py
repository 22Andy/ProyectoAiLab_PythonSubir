...
class Combate_entrenamiento():
	def __init__(self, nombre1,nombre2,tipo1, tipo2):
	...
	def combate(self,poder):
		...
		if (poder == self.diccionario[self.nombre1]['Potenciador']) and  (self.band_pot1=="Desactivado")  and (self.contador_poten1==0):
			...
			self.poten_1 = poder
			self.n.config(text=self.nombre1+''' activo '''+self.poten_1+'''\n Sus ataques se intensifican por 2 turnos''',anchor="center",fg='blue')
		...
		if self.puntaje_rival<=0:
			messagebox.showinfo(message=self.nombre1+" Gano!!!", title="Felicitaciones")
			self.n.config(text= "Se acabo la batalla" )
			self.ventana.update()
			time.sleep(2)
	...
	def CPU(self):
		...
		if self.puntaje_personaje <=0:
			messagebox.showinfo(message='''Gano '''+self.nombre2+", jugador del CPU !!!!!", title="Felicitaciones")
			self.n.config(text= "Se acabo la batalla",anchor= "center",fg='black')
			self.ventana.update()
			time.sleep(2)
			self.ventana.destroy()
			ME.entrenamiento()
	...
	def pantalla_sesion(self):
		...
		self.n = Button(self.ventana, text="El juego esta por iniciar",bg='pink',fg='blue', width=45)
